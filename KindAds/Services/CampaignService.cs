...
namespace KindAds.Services
{
  public class CampaignService : ITelemetria
  {
    ...
    {
      if (fileUpload != null) { return Helpers.AzureStorageHelper.CreateBlobImageFile(fileUpload.FileData, fileUpload.Filextension); } else { return ""; }
    }
    ...
    {
      List<int> status = new List<int>() { (int)CatCampaignStatusEnum.In_Review, (int)CatCampaignStatusEnum.Waiting_For_Response, (int)KindAds.Comun.Enums.CatCampaignStatusEnum.Verify_Proccess };
