...
{
  public class IContactService<TCampaign, TR> : ITelemetria, IIContactService<TCampaign> where TCampaign : ICampaign where TR : IResponse, new()
  {
    #region properties
    ...
    public IContactRequest FillIContactRequest(ICollection<PRODUCT_SETTINGS> settings)
    {
    ...
    public IResponse CreateMessageToHttpClient(IContactPostMessageRequest requestBody, IContactRequest requestFrm)
    {
      ...
      try
      {
        ...
        string uri = requestManager.GetUriRequest(KindAds.Common.Utils.IContactRequest.AddMessage, config, requestFrm);
        HttpClient client = requestManager.GetHttpClient(config, KindAds.Common.Utils.IContactRequest.AddMessage);
