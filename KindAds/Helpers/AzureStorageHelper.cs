...
namespace KindAds.Helpers
{
  ...
  {
    public static string CreateBlobImageFile(byte[] sdata, string filextension, int width = 480, int height=320)
    {
      CloudStorageAccount storageAccount = CloudStorageAccount.Parse(Utils.Configuration.AppSettings.azurestorageconnectionstring);
      ...
      byte[] resized_image = Helpers.FileHelper.ResizeImageSite(sdata,width,height);
    ...
    public static string CreateHtmlTemplateAzure(string htmlText)
    {
      string filextension = ".html";
            CloudStorageAccount storageAccount = CloudStorageAccount.Parse(Utils.Configuration.AppSettings.azurestoragehtmltemplateconnectionstring);
