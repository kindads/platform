...
namespace captivate_express_webapp.Helpers
{
  public static class FileHelper
  {
    ...
    public static byte[] ResizeImageSite(byte[] OriginalImage)
    {
      ...
      if (ancho > 480 || alto > 320)
      {
        if (ancho > alto)
        {
          factor = (Decimal)480 / ancho;
        }
        else
        {
          factor = (Decimal)320 / alto;
        }
      }
      else { factor = 1; }
        ...
        Graphics thumbnailGraph = null;
          using (thumbnailBitmap = new Bitmap(newWidth, newHeight))
          {
            using (thumbnailGraph = Graphics.FromImage(thumbnailBitmap))
            {
              ...
              thumbnailBitmap.Save(ResultStream, image__1.RawFormat);
            }
          }
