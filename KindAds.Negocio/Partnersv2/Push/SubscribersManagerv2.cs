...
{
    public class SubscribersManagerv2 : BaseManager, IPushChannel
    {
        #region properties
        public IList<AudiencePropertieSetting> settings { set; get; }
        public bool AreSettingsValid { set; get; }
        public SubscribersManager manager { set; get; }
        ...
        public void UpdateUrlToValidate(string Url)
        {
            this.Url = Url;
        }
        ...
        public bool ValidateApiToken(string ApiToken)
        {
            ...
            {
                result = manager.ValidateKey(ApiToken, Url);
            }           
            return result;
        }
        ...
        #region constructors
        public SubscribersManagerv2() { }

        public SubscribersManagerv2(string emailProductId)
        {
            LoadSettings(emailProductId);
            if (settings != null && settings.Count > 0)
            {
                this.manager = new SubscribersManager(settings);
            }

            // command pattern
            AreSettingsValid = SettingsAreValid(emailProductId);
            if (AreSettingsValid)
            {
                Save(emailProductId);
            }
        }
        public SubscribersManagerv2(IList<AudiencePropertieSetting> settings, string emailProductId)
        {
