...
using KindAds.Business.Communication;
...
using KindAds.Negocio.Communication;
using KindAds.Azure;

namespace KindAds.Negocio.Partners.Mail
{
    public class IContacLogic<TCampaign, TR> : ITelemetria, IIContactService<TCampaign> where TCampaign : ICampaign where TR : IResponse, new()
    {
        ...
        #region properties validation method


        [Obsolete]
        public List<ProductSettingsEntity> GetProductSettingEntity(string IdCampaign)
        {
        ...
        public bool UsernameIsValid(string idCampaign)
        {
            bool result = false;
            ...
            try
            {
                ...
                config.mailingProvider = requestFrm;
                HttpWebRequest request = GetRequest(Common.Utils.IContactRequest.GetUsers, config, requestFrm);
                response = GetResponse(request);
        ...
        public bool PasswordIsValid(string idCampaign)
        {
            bool result = false;
        ...
        public bool ApiAppIsValid(string idCampaign)
        {
            bool result = false;
        ...
        public bool AccountIsValid(string idCampaign)
        {
            bool result = false;
        ...
        public bool FolderIsValid(string idCampaign)
        {
            bool result = false;
            ...
            try
            {
                ...
                IContactRequest requestFrm = FillIContactRequest(settings);
                config.mailingProvider = requestFrm;
                HttpWebRequest request = GetRequest(Common.Utils.IContactRequest.GetFolders, config, requestFrm);
                response = GetResponse(request);
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    result = true;
