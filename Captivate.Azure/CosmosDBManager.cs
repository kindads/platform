...
namespace Captivate.Azure
{
    public class CosmosDBManager : ITelemetria
    {
        ...
        {
            EndpointUrl = ConfigurationManager.AppSettings["CosmosDB-EndpointUrl"];
            PrimaryKey = ConfigurationManager.AppSettings["CosmosDB-PrimaryKey"];
        ...
        public bool CreateDocumentIfNotExists<T>(string DatabaseName, string CollectionName, T Document) where T : CosmosDbDocument
        {
            bool result = false;
            try
            {
        ...
        public List<T> GetAllDocuments<T>(string DatabaseName, string CollectionName) where T: CosmosDbDocument, new()
        {
